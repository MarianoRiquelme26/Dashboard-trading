...
import type { JournalTradeItem, SignalTradeLink } from "@/types/snapshots"
...
function LinkDetail({ trade, link }: { trade: JournalTradeItem; link: SignalTradeLink }) {
...
export function TradeDetailPanel({ trade }: { trade: JournalTradeItem | null }) {
  ...
  const links = trade.tradeLinks ?? []
  const link = links[0] ?? trade.tradeLink ?? null
  const linkStatus = linkStatusForTrade(trade)
  const hasTradeTest = hasExplicitTestMetadata(trade)
    ...
    <aside className="rounded-lg border border-border bg-card p-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <div className="flex flex-wrap items-center gap-2">
            ...
            {(hasTradeTest || hasAnyLinkTest) && <StatusPill label="TEST" tone="mock" />}
          </div>
      ...
      {hasTradeTest && (
        <div className="mt-4 rounded-md border border-fuchsia-300 bg-fuchsia-50 p-3 dark:border-fuchsia-500/30 dark:bg-fuchsia-500/10">
          <Field label="Test case" value={testCaseText(trade)} />
        </div>
      )}
      ...
      {link ? (
        <LinkDetail trade={trade} link={link} />
      ) : (
        <>
          <div className="mt-4 rounded-md border border-border bg-secondary/20 p-3">
            <p className="font-heading text-sm font-semibold text-foreground">Senal vinculada</p>
            <p className="mt-2 text-sm text-muted-foreground">
              {hasTradeTest ? "Trade QA sin senal vinculada." : "Trade sin senal vinculada."}
            </p>
          </div>
          <p className="mt-4 rounded-md border border-border bg-secondary/20 p-3 text-sm text-muted-foreground">
            No se muestra comparacion plan vs ejecucion porque no existe link real.
          </p>
        </>
      )}
    </aside>
